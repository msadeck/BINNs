...
class D_MLP(nn.Module):
    ...
    def __init__(self, input_features=1, scale=1):
        
        super().__init__()
        self.inputs = input_features
        self.scale = scale
        self.min = 0 #/ (1000**2) / (1/24) # um^2/hr -> mm^2/d
        self.max = 0.45 #/ (1000**2) / (1/24) # um^2/hr -> mm^2/d
        self.mlp = BuildMLP(
...
class BINN(nn.Module):
    ...
    def __init__(self, delay=False):
        ...
        self.pde_weight = 1e0
        self.D_weight = 1e0 / self.D_max
        self.G_weight = 1e0 / self.G_max
        self.T_weight = 1e10 
    ...
    def pde_loss(self, inputs, outputs, return_mean=True):
        ...
        pde_residual = (LHS - RHS)**2

        # Save PDE residual mean loss (this is the part you want as self.pde_loss)
        self.pde_loss_log = torch.mean(pde_residual)
        ...
        try:
            ...
            self.D_loss += self.dDdu_weight*torch.where(
                dDdu < 0.0, 2*torch.ones_like(dDdu), torch.zeros_like(dDdu))
        except:
            pass
        try:
        ...
        if return_mean:
            return torch.mean(pde_residual + self.D_loss + self.G_loss + self.T_loss)
        else:
            return self.pde_loss_log + self.D_loss + self.G_loss + self.T_loss
